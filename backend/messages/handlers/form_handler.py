...
from backend.utils.user_utils import criar_usuario
...
from backend.utils.register_utils import vincular_professor_by_telegram, vincular_aluno_by_telegram, realizar_login

pwd_context = CryptContext(schemes=["argon2"], deprecated="auto")

def register(app: Client):
    @app.on_message(filters.text & ~filters.command(["start", "register", "logar", "professores", "projetos", "universidades", "vincular"]))
    async def form_handler(client: Client, message: Message):
        ...
        if state == "esperando_departamento_professor":
            ...
            await vincular_professor_by_telegram(
                telegram_id=telegram_id,
        ...
        if state == "esperando_matricula_aluno":
            ...
            await vincular_aluno_by_telegram(
                telegram_id=telegram_id,
                universidade_id=data["universidade_id"],
